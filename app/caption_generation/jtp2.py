"""
Joint Tagger Project PILOT2 (JTP2) caption generator implementation.
...
class JTP2Generator(CaptionGenerator):
    ...
    def _create_helper_classes(self):
        ...
        class CompositeAlpha(torch.nn.Module):
            def __init__(self, background: tuple[float, float, float] | float):
                ...
                self.background = (
                    torch.tensor(self.background).unsqueeze(1).unsqueeze(2)
                )
    ...
    def _generate_sync(self, image_path: Path) -> str:
        ...
        # Process results
        tag_score = {
            self._tags[indices[i]]: values[i].item() for i in range(indices.size(0))
        }
        filtered_tags = [
            tag for tag, score in tag_score.items() if score > self.threshold
        ]
