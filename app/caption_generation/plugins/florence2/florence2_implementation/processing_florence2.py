...
class Florence2PostProcesser(object):
        ...
        for pharse_text in phrases:
            ...
            if score_mode == "avg_loc_scores":
                if spans is None or scores is None:
                    ...
                    bbox_end_spans = [
                        _bboxes_parsed.span(0) for _bboxes_parsed in bboxes_parsed
                    ]
                    all_scores = []
                    for _spans in bbox_end_spans:
                        token_inds = find_matched_token_indices(
                            (_spans[0] + cur_span, _spans[1] + cur_span), spans
                        )
                ...
                if spans is None or scores is None:
                    ...
                    cat_name_scores = [
                        scores[token_i] for token_i in cat_name_token_inds
                    ]
    ...
    def __call__(
        ...
        assert (
            sequence is not None or text is not None
        ), "sequence or text should be provided"
        assert (
            sequence is None or text is None
        ), "only one of sequence and text should be provided"
