...
# Set default RUN_MODE to 'both' if not specified
...
if [ "$RUN_MODE" = "backend" ]; then
  ...
  if [ "$NODE_ENV" = "production" ]; then
    echo "RUN_MODE is both and NODE_ENV is production. Starting backend only (production static files will be served by backend)."
    python3 -m uvicorn app.main:app --host 0.0.0.0 --port ${PORT:-7000}
  else
